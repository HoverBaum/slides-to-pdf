  ...
  const tasks = new Listr([
    {
      ...
      task: ctx => new Promise(async (resolve, reject) => {
        try {
          await page.goto(`${baseUrl}/${firstSlideNumber}`)
    ...
    },
    {
      ...
      task: (ctx, task) => new Promise(async (resolve) => {
        const startSlideIndex = !firstSlideNumber ? 0 : parseInt(firstSlideNumber)
        let currentSlide = startSlideIndex
        const totalNumberOfSlides = ctx.lastSlideNumber - startSlideIndex + 1
        while (currentSlide <= ctx.lastSlideNumber) {
          ...
          const slideCount = currentSlide - startSlideIndex + 1
          task.output = `Saving slide ${slideCount}/${totalNumberOfSlides}`
          const pathToVisit = currentSlide === 0 ? '' : currentSlide
          const urlToVisit = `${baseUrl}/${pathToVisit}`
