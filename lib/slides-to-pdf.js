  ...
  const tasks = new Listr([
    {
      ...
      task: ctx => new Promise(async (resolve, reject) => {
        try {
          await page.goto(`${baseUrl}/${!hasNoFirstNumber && firstSlideNumber}`)
          ctx.lastSlideNumber = await findLastSlideNumber(page)
          resolve()
        } catch (e) {
          reject(e)
        }
    ...
    },
    {
      ...
      task: (ctx, task) => new Promise(async (resolve) => {
        ...
        let currentSlide = firstSlideNumber
        const totalNumberOfSlides = ctx.lastSlideNumber - firstSlideNumber + 1
        while (currentSlide <= ctx.lastSlideNumber) {
          ...
          const slideCount = firstSlideNumber === 0 ? currentSlide + 1 : currentSlide - firstSlideNumber + 1
          task.output = `Saving slide ${slideCount}/${totalNumberOfSlides}`
          const pageToVisit = hasNoFirstNumber && currentSlide === firstSlideNumber ? '' : currentSlide
          const urlToVisit = `${baseUrl}/${pageToVisit}`
          ctx.urls = ctx.urls ? ctx.urls.concat([urlToVisit]) : [urlToVisit]
          await page.goto(urlToVisit)
          const filePath = path.join(tmpFolder, `page${currentSlide}.pdf`)
